...
install_and_configure_traefik2() {
  ...
  TRAEFIK_PV=/root/traefik2_pv.yaml
  render_traefik2_volume
  kubectl apply -f $TRAEFIK_PV --namespace=traefik
...
spec:
  ...
  accessModes:
    - ReadWriteOnce
  hostPath:
    path: /mnt/storage/traefik/acme/
EOF
}

render_traefik2_config() {
cat << 'EOF' > "$TRAEFIK_VALUES_FILE"
# Set init containers to 
deployment:
  initContainers:
    - name: volume-permissions
      ...
      command:
        [
          "sh",
          "-c",
          "touch /acme/acme.json; chown 65532 /acme/acme.json; chmod -v 600 /acme/acme.json",
        ]
      securityContext:
        runAsNonRoot: false
        runAsGroup: 0
...
render_cloud_provider_example_oci_csi_secret() {
CCM_CSI_FILE=/root/oci_ccm_csi.yaml
cat << EOF > "$CCM_CSI_FILE"
...
instance_info=$(curl -s -H "Authorization: Bearer Oracle" -L http://169.254.169.254/opc/v2/instance)
instance_id=$(echo $instance_info | jq -r '.displayName')
instance_ocid=$(echo $instance_info | jq -r '.id')
# instance_fault_domain=$(echo $instance_info | jq -r '.faultDomain' )

vnic_info=$(curl -s -H "Authorization: Bearer Oracle" -L http://169.254.169.254/opc/v2/vnics/)
vnic_id=$(echo $vnic_info | jq -r '.[].vnicId')
...
k3s_install_params+=("--kube-controller-manager-arg cloud-provider=external")
...
# Enable kubectl on server
sudo mkdir -p ~/.kube/ && sudo cp /etc/rancher/k3s/k3s.yaml ~/.kube/config && sudo chown $USER ~/.kube/config && chmod 600 ~/.kube/config && export KUBECONFIG=~/.kube/config

# Install OCI CCM
%{ if install_oci_ccm }
# If this is the first instance - add ccm config file and create a secret
if [[ "$first_instance" == "$instance_id" ]]; then

  # Render config file for CCM and CSI
  render_cloud_provider_example_oci_csi_secret
  
  # Remove Taint the node to allow scheduling
  kubectl taint nodes $instance_id node.cloudprovider.kubernetes.io/uninitialized="true":NoSchedule-

  #Create CCM secret
  kubectl create secret generic oci-cloud-controller-manager -n kube-system --from-file=cloud-provider.yaml=$CCM_CSI_FILE
  
  #Download CCM config files
  curl -L https://github.com/oracle/oci-cloud-controller-manager/releases/download/${ccm_release}/oci-cloud-controller-manager-rbac.yaml -o oci-cloud-controller-manager-rbac.yaml
  curl -L https://github.com/oracle/oci-cloud-controller-manager/releases/download/${ccm_release}/oci-cloud-controller-manager.yaml -o oci-cloud-controller-manager.yaml

  #Parse oci-ccm to fix nodeSelector
  sed -i 's|        node-role.kubernetes.io/control-plane: ""|        node-role.kubernetes.io/control-plane: "true"|g' /oci-cloud-controller-manager.yaml

  #Install the OCI CCM
  kubectl apply -f oci-cloud-controller-manager-rbac.yaml
  kubectl apply -f oci-cloud-controller-manager.yaml

  #Install and configure OCI CSI
  #Create CSI secret
  kubectl create secret generic oci-volume-provisioner -n kube-system --from-file=config.yaml=$CCM_CSI_FILE

  curl -L "https://github.com/oracle/oci-cloud-controller-manager/releases/download/${ccm_release}/oci-csi-node-rbac.yaml" -o oci-csi-node-rbac.yaml
  curl -L "https://github.com/oracle/oci-cloud-controller-manager/releases/download/${ccm_release}/oci-csi-controller-driver.yaml" -o oci-csi-controller-driver.yaml
  curl -L "https://github.com/oracle/oci-cloud-controller-manager/releases/download/${ccm_release}/oci-csi-node-driver.yaml" -o oci-csi-node-driver.yaml
  ...
  #Parse oci-csi-controller-driver to fix the nodeSelector
  sed -i 's|node-role.kubernetes.io/control-plane: ""|node-role.kubernetes.io/control-plane: "true"|g' /oci-csi-controller-driver.yaml
...
#Set labels on each node
  availability_trimmed=$(echo ${availability_domain} | cut -d ":" -f 2)
  kubectl label node $instance_id failure-domain.beta.kubernetes.io/zone=$availability_trimmed --overwrite

#   kubectl label node $instance_id topology.kubernetes.io/zone=$availability_trimmed --overwrite
#   kubectl label node $instance_id oci.oraclecloud.com/fault-domain=$instance_fault_domain --overwrite
#   kubectl label node $instance_id failure-domain.beta.kubernetes.io/region=${region} --overwrite
#   kubectl label node $instance_id topology.kubernetes.io/region=${region} --overwrite
#   kubectl annotate node $instance_id oci.oraclecloud.com/compartment-id=$compartment_ocid
